...
def test_load_vision_refuses_what_would_drop_the_image():
    """load_vision refuses, before loading anything, each way an image record could run without its image: a non-torch
    backend, CUDA graphs (they replay token ids), the fused kernels, and a full-weight checkpoint (a text backbone alone)."""
    from kev.checkpoint import Checkpoint, LoadOptions
    class Adapter(Checkpoint): path, full = "runs/adapter", False
    class Full(Checkpoint): path, full = "runs/full", True
    for cls, opts in ((Adapter, LoadOptions(backend="mlx")), (Adapter, LoadOptions(cuda_graphs=True)),
                      (Adapter, LoadOptions(fused=True)), (Full, LoadOptions())):
        with pytest.raises(ValueError):
            cls.__new__(cls).load_vision("cpu", opts)
