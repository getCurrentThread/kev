...
- `kev/vision.py`    VisionDecisionModel (`Checkpoint.load_vision`): an image spliced after `<state>`, the adapter + head on the language model inside the base's vision-language model, rows with the base's M-RoPE positions (`_rows_hidden(inputs=...)`); torch only, no serving or prefix cache; parity in `scripts/vision_parity.py`
