...
- Unit tests (no weights; the CI `python` job): `uv run --extra serve python -m pytest tests/test_unit.py tests/test_research.py tests/test_generators.py tests/test_conventions.py -q`.
  ...
  (`runs/smoke-hl/00-trial-0/checkpoint` + Qwen2.5-0.5B, Qwen3.5-0.8B-Base and `jaredpalmer/kev-0.8b` downloads, ~5 min, local only): `tests/test_model.py`; `tests/test_mlx.py` (Apple Silicon, ~30 s) for the MLX backend.
...
- `kev/vision.py`    VisionDecisionModel (`Checkpoint.load_vision`): the same adapter + head on the language model inside the base's vision-language model (`DecisionModel.backbone` hook); an image spliced after `<state>` (`splice_image`, at most `MAX_IMAGE_PIXELS`), the row form through `_rows_hidden(inputs=...)` with the base's M-RoPE positions, vision tower once per record; no serving, prefix cache or MLX. `scripts/vision_parity.py` measures it against the base's own multimodal forward
...
- `scripts/`         one-off builders and read-outs: `calibrate_checkpoint.py`, `vision_parity.py`, `build_night2_data.py`, `build_binding_diagnostic.py`,
